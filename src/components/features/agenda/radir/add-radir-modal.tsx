...
export function AddRadirModal({ dirOptions, pemOptions, supOptions }: AddRadirModalProps) {
    ...
    const handleSupportChange = (newValue: MultiValue<UnitOption>) => {
        ...
        // Hapus duplikasi jika ada (misal user memilih item yang sama lagi dari dropdown)
        ...
        try {
            ...
            for (const doc of FILE_LIST) {
                ...
                if (file && file.size > 0) {
                    ...
                    // Simpan nama asli file (sanitized) agar bisa ditampilkan di UI
            ...
            if (result.success) {
                showNotify(
                    isDraft ? "Data tersimpan lokal sebagai Draft." : "Agenda telah masuk antrian verifikasi.",
                    "success"
                );
                ...
                showNotify("Gagal menyimpan data ke database.", "error")
            }
            ...
            <DialogContent className="max-w-[95vw] sm:max-w-3xl h-[90vh] max-h-[90vh] p-0 flex flex-col gap-0 border-none shadow-2xl rounded-xl bg-white overflow-hidden">
                <DialogHeader className="px-6 py-5 bg-[#006070] text-white shrink-0 z-10">
                    <DialogTitle className="text-xl font-bold flex items-center gap-2">
                        ...
                        Form Usulan Agenda RADIR
                    </DialogTitle>
                    <DialogDescription className="text-slate-100/80 text-sm">
                        Lengkapi formulir di bawah ini untuk mengajukan agenda rapat baru.
                    </DialogDescription>
                </DialogHeader>
                ...
                <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0 overflow-hidden">
                    <div className="flex-1 overflow-y-auto bg-slate-50/50 px-6 py-6 scroll-smooth">
                        <div className="grid gap-8 pb-4">
                                ...
                                <div className="grid gap-4">
                                    ...
                                    <div className="space-y-1.5">
                                        ...
                                        <Textarea
                                            name="urgency"
                                            value={urgensiText}
                                            onChange={(e) => setUrgensiText(e.target.value)}
                                            required
                                            placeholder="Jelaskan kenapa agenda ini penting..."
                                            className="min-h-20 bg-white border-slate-300 focus:border-[#006070]"
                                        />
                                    </div>
                            ...
                            {/* --- SEKSI 2: STAKEHOLDERS (PROPS DRIVEN) --- */}
                            <section className="space-y-4">
                                <div className="flex items-center gap-2 border-b border-slate-200 pb-2">
                        ...
                        <div className="flex gap-2">
                            ...
                            <Button type="submit" disabled={isPending} className={`min-w-35 font-bold shadow-md transition-all ${isDraft ? "bg-yellow-500 hover:bg-yellow-600 text-white" : "bg-[#006070] hover:bg-[#004d5a] text-white"}`}>
                                {isPending ? (
                                    ...
                                    <><Send className="mr-2 h-4 w-4" /> Kirim Usulan</>
                                )}
                            </Button>
                        </div>
                    </DialogFooter>
