...
import { columns } from "./columns"; // ✅ Import columns yang berisi checkbox
import { DeleteRadirDialog } from "./delete-radir-dialog"; // ✅ Import Dialog Hapus
...
export function AgendaMain({ initialData }: AgendaMainProps) {
    ...
    // NOTE: React Compiler warning: useReactTable returns non-memoizable functions.
    // This is expected and safe as long as 'table' is not passed to memoized components/hooks.
    // See: https://tanstack.com/table/v8/docs/guide/react/react-compiler
    ...
    // ✅ Handler Hapus Bulk (Dipanggil dari Toolbar)
        ...
        <div className="flex flex-col h-full space-y-4">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div className="flex-1">
                    <AgendaToolbar
                        ...
                        onDateChange={(date) => {
                            ...
                            if (date) table.getColumn("deadlineDate")?.setFilterValue(date.toISOString().split('T')[0]);
                            else table.getColumn("deadlineDate")?.setFilterValue(undefined);
                        }}
                        onReset={() => table.resetColumnFilters()}
                        // ✅ Pass data seleksi ke Toolbar
                ...
                <div className="flex items-center gap-1 rounded-lg bg-slate-100 p-1 border border-slate-200 h-fit">
                    <Button
                        ...
                        className={cn("h-8 w-8 px-0", viewMode === "list" && "bg-white text-[#006070] shadow-sm")}
                    >
                        <List className="h-4 w-4" />
                    </Button>
                    <Button
                        variant="ghost" size="sm" onClick={() => setViewMode("grid")}
                        className={cn("h-8 w-8 px-0", viewMode === "grid" && "bg-white text-[#006070] shadow-sm")}
                    >
            ...
            <div className="flex-1 overflow-hidden rounded-xl border bg-white shadow-sm relative min-h-[50vh]">
                {viewMode === "list" ? (
                    ...
                    <div className="bg-slate-50/50 h-full overflow-y-auto">
                        {/* Gunakan getRowModel() agar Grid juga ter-paginasi sesuai Table */}
                        <AgendaGrid data={table.getRowModel().rows.map(r => r.original)} />
                    </div>
                )}
            </div>
            ...
            <DeleteRadirDialog
                ...
                ids={deleteIds}
                onSuccess={() => {
                    setRowSelection({}) // Reset seleksi setelah hapus
                    router.refresh()
                }}
            />
        </div>
