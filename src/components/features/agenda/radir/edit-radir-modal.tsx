...
import { updateRadirAction, getAgendaById } from "@/server/actions/radir-actions"
import { getUnitsByCategory } from "@/server/master-data-actions"
import { createClient } from "@/lib/supabase/client"
import { DeleteRadirDialog } from "./delete-radir-dialog" // ✅ Import Dialog
...
export function EditRadirModal({ open, onOpenChange, agendaId }: EditRadirModalProps) {
    ...
    const [supOptions, setSupOptions] = useState<{ label: string; value: string }[]>([])
    ...
    // Original Status State (untuk mempertahankan status akhir)
    ...
    // ✅ NEW: State untuk Konfirmasi Delete
    const [fileToDelete, setFileToDelete] = useState<string | null>(null) // Menyimpan ID dokumen yang akan dihapus
    const [showDeleteAgenda, setShowDeleteAgenda] = useState(false) // ✅ State Dialog Hapus Agenda
    ...
    useEffect(() => {
        if (open) {
            const fetchMaster = async () => {
                ...
                const [dirs, pems, sups] = await Promise.all([
                    getUnitsByCategory("DIREKTUR_PEMRAKARSA"),
                    getUnitsByCategory("PEMRAKARSA"),
                    getUnitsByCategory("SUPPORT")
                ])
                setDirOptions(dirs); setPemOptions(pems); setSupOptions(sups);
            }
            fetchMaster()
        }
    ...
    useEffect(() => {
        if (open && agendaId) {
            ...
            const fetchAgenda = async () => {
                const rawData = await getAgendaById(agendaId)
                const data = rawData as unknown as AgendaData

                if (data) {
                    setJudul(data.title ?? "")
                    setUrgensi(data.urgency ?? "")
                    // Gunakan deadlineDate untuk form edit
                    const dateStr = data.deadlineDate ? new Date(data.deadlineDate).toISOString().split('T')[0] : "";
                    setDeadline(dateStr || "");
                    setOriginalStatus(data.status ?? "") // Simpan status asli
                    ...
                    if (data.support) setSelectedSupport(data.support.split(", ").map(s => ({ label: s, value: s })))
                    else setSelectedSupport([])
                }
    ...
    const handleSupportChange = (newValue: MultiValue<{ label: string; value: string }>) => {
    ...
    // ✅ Fungsi Eksekusi Hapus File
    const handleConfirmDelete = () => {
        if (fileToDelete) {
            const newFiles = { ...existingFiles };
            delete newFiles[fileToDelete]; // Hapus dari state
            setExistingFiles(newFiles);

            setFileToDelete(null); // Tutup dialog
        ...
        try {
            ...
            for (const doc of FILE_LIST) {
                ...
                if (file && file.size > 0) {
                    ...
                    // Simpan nama asli file (sanitized)
                    const sanitizedName = file.name.replace(/[^a-zA-Z0-9.\-_]/g, '_');
                    const fileName = `${doc.id}-${uuidv4()}-${sanitizedName}`;
                    const filePath = `radir/${fileName}`;
            ...
            if (supportingFiles.length > 0) {
                ...
                for (const file of supportingFiles) {
                    if (file && file.size > 0) {
                        ...
                        const filePath = `radir/supporting/${fileName}`;
            ...
            // Logic Handling Data (Sama seperti sebelumnya)
            // Pastikan urgencyDesc menangkap nilai 'urgency' (Textarea)
            const urgencyDesc = formData.get("urgency") as string;
            formData.set("urgencyDesc", urgencyDesc);
            ...
            // Logic Status:
            // - Jika status asli adalah "Selesai", "Dijadwalkan", "Dibatalkan", atau "Ditunda" -> pertahankan status
            // - Jika status asli "Draft" dan isDraft true -> tetap "Draft"
            // - Jika status asli "Draft" dan isDraft false -> ubah ke "Dapat Dilanjutkan"
            // - Selain itu, gunakan logika isDraft
            const lockedStatuses = ["Selesai", "Dijadwalkan", "Dibatalkan", "Ditunda"];
            let finalStatus: string;
            if (lockedStatuses.includes(originalStatus)) {
                finalStatus = originalStatus; // Pertahankan status akhir
            } else {
                finalStatus = isDraft ? "Draft" : "Dapat Dilanjutkan";
            }
            formData.append("status", finalStatus)

            // Prioritas (dikirim sebagai metadata jika perlu, tapi urgencyDesc yang utama)
            ...
            const result = await updateRadirAction(agendaId, formData)
                ...
                <DialogContent className="max-w-[95vw] sm:max-w-3xl h-[90vh] max-h-[90vh] p-0 flex flex-col gap-0 border-none shadow-2xl rounded-xl bg-white overflow-hidden">
                    <DialogHeader className="px-6 py-5 bg-[#006070] text-white shrink-0 z-10">
                        ...
                        <DialogDescription className="text-slate-100/80 text-sm">
                            Perbarui informasi agenda rapat di bawah ini.
                        </DialogDescription>
                    </DialogHeader>
                    ...
                    {isLoadingData ? (
                        ...
                        <form onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0 overflow-hidden">
                                ...
                                <div className="grid gap-8 pb-4">
                                    ...
                                    <section className="space-y-4">
                                        ...
                                        <div className="grid gap-4">
                                            ...
                                            <div className="space-y-1.5">
                                                <Label className="text-[#006070] font-semibold">Urgensi</Label>
                                                <Textarea name="urgency" value={urgensi} onChange={(e) => setUrgensi(e.target.value)} required className="min-h-20 bg-white border-slate-300 focus:border-[#006070]" />
                                            </div>
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                ...
                                                <div className="space-y-1.5">
                                                    <Label className="text-[#006070] font-semibold">Prioritas</Label>
                                                        ...
                                                        {!notRequiredFiles.includes(doc.id) && (
                                                            <>
                                                                {existingFiles[doc.id] ? (
                                                                    // ✅ JIKA FILE ADA: Tampilkan Badge & Tombol Hapus
                                                                    <div className="flex items-center gap-2 animate-in fade-in zoom-in-95 duration-200">
                                                                        ...
                                                                        <Button
                                                                            ...
                                                                            onClick={() => setFileToDelete(doc.id)} // Trigger Dialog Delete
                                                                            title="Hapus untuk mengganti file"
                                                                        >
                                                                            <Trash2 className="h-4 w-4" />
                                                                        </Button>
                                                                    </div>
                                                                ) : (
                                                                    // ✅ JIKA FILE KOSONG/DIHAPUS: Tampilkan Input
                            ...
                            <DialogFooter className="p-4 bg-white border-t shrink-0 flex items-center justify-between z-10">
                                {/* ✅ KIRI: Tombol Hapus Agenda (Hanya jika Edit Mode) */}
                                {agendaId && (
                                    <Button
                                        type="button"
                                        variant="destructive"
                                        onClick={() => setShowDeleteAgenda(true)}
                                        className="mr-auto bg-red-50 text-red-600 hover:bg-red-100 border border-red-200 shadow-none"
                                    >
                                ...
                                <div className="flex items-center space-x-2 bg-yellow-50 px-3 py-2 rounded-md border border-yellow-200">
                                    ...
                                    />
                                    <Label htmlFor="draftEdit" className="text-sm font-medium text-yellow-800 cursor-pointer select-none">
                ...
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription className="text-slate-600">
                            Apakah Anda yakin ingin menghapus dokumen ini? <br />
                            <span className="font-semibold text-slate-800">Tindakan ini tidak dapat dibatalkan.</span> Anda harus mengunggah dokumen baru sebagai pengganti.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
