...
import { scheduleUnifiedAgendas } from "@/server/actions/jadwal-rapat-actions"
import { useRouter } from "next/navigation"

interface ScheduleDialogProps {
    open: boolean
    onOpenChange: (open: boolean) => void
    selectedAgendas: { id: string; title: string; meetingType: "RADIR" | "RAKORDIR" }[]
    onSuccess?: () => void
}
...
export function ScheduleDialog({ open, onOpenChange, selectedAgendas, onSuccess }: ScheduleDialogProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setIsPending(true)
        try {
            const items = selectedAgendas.map(a => ({ id: a.id, meetingType: a.meetingType }))

            const result = await scheduleUnifiedAgendas(items, {
                executionDate: date,
                startTime,
                endTime,
                        ...
                        <ScrollArea className="h-24 pr-2">
                            <ul className="space-y-1">
                                {selectedAgendas.map((agenda) => (
                                    <li key={agenda.id} className="text-xs text-slate-700 list-disc list-inside line-clamp-1 flex gap-2">
                                        <span className="font-bold text-[10px] bg-slate-200 px-1 rounded">{agenda.meetingType}</span>
                                        {agenda.title}
                                    </li>
                                ))}
                            </ul>
                            ...
                            <div className="space-y-1.5">
                                ...
                                <div className="flex gap-2">
                                    <div className="relative flex-1">
                                        <Clock className="absolute left-2.5 top-2.5 h-4 w-4 text-slate-400" />
                                        <Input type={isCustomEndTime ? "time" : "text"} className="pl-9" value={endTime} onChange={(e) => setEndTime(e.target.value)} readOnly={!isCustomEndTime} placeholder="Selesai" />
                                    </div>
                                    <Button type="button" variant="outline" size="icon" className="shrink-0" onClick={() => { if (isCustomEndTime) { setIsCustomEndTime(false); setEndTime("Selesai"); } else { setIsCustomEndTime(true); setEndTime(""); } }} title={isCustomEndTime ? "Set ke 'Selesai'" : "Set Jam Selesai"}>
                            ...
                            {(method === "OFFLINE (Tatap Muka)" || method === "HYBRID (Campuran)") && (
                                <div className="space-y-1.5">
                                    ...
                                    <div className="relative">
                                        ...
                                        <Input className="pl-9" placeholder="Contoh: Ruang Rapat Lt. 3" value={location} onChange={(e) => setLocation(e.target.value)} />
                                    </div>
                                </div>
                            )}
                            {(method === "ONLINE (Daring)" || method === "HYBRID (Campuran)") && (
                                <div className="space-y-1.5">
                                    ...
                                    <div className="relative">
                                        ...
                                        <Input className="pl-9" placeholder="https://zoom.us/j/..." value={link} onChange={(e) => setLink(e.target.value)} />
                                    </div>
                                </div>
                            )}
                        </div>
