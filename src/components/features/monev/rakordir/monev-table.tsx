...
// src/components/features/monev/rakordir/monev-table.tsx
"use client";
...
interface MonevTableProps {
    data: MonevRakordirItem[];
}

export function MonevRakordirTable({ data }: MonevTableProps) {
    ...
    // Stats - count total arahan direksi across all agendas
    const totalAgendas = data.length;
    const totalArahan = data.reduce((sum, d) => sum + d.totalArahan, 0);
    const completedArahan = data.reduce((sum, d) => sum + d.completedArahan, 0);
    const inProgressCount = totalArahan - completedArahan;
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-4 gap-4">
                <div className="bg-white rounded-lg border p-4 shadow-sm">
                    <div className="flex items-center gap-3">
                        <div className="p-2 rounded-lg bg-[#e7f6f9]">
                            <ClipboardList className="h-5 w-5 text-[#125d72]" />
                        </div>
                        <div>
                ...
                <div className="bg-white rounded-lg border p-4 shadow-sm">
                    <div className="flex items-center gap-3">
                        <div className="p-2 rounded-lg bg-[#e7f6f9]">
                            <ClipboardList className="h-5 w-5 text-[#14a2ba]" />
                        </div>
                        <div>
                            <p className="text-2xl font-bold text-[#14a2ba]">{totalArahan}</p>
                            <p className="text-xs text-muted-foreground">Total Arahan</p>
                        </div>
                    </div>
                </div>
                <div className="bg-white rounded-lg border p-4 shadow-sm">
                    <div className="flex items-center gap-3">
                        <div className="p-2 rounded-lg bg-[#efe62f]/20">
                            <Clock className="h-5 w-5 text-orange-600" />
                        </div>
                        <div>
                            <p className="text-2xl font-bold text-[#b5a800]">{inProgressCount}</p>
                ...
                <div className="bg-white rounded-lg border p-4 shadow-sm">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <p className="text-2xl font-bold text-green-600">{completedArahan}</p>
            ...
            <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between bg-white rounded-lg border p-4 shadow-sm">
                <div className="relative flex-1 max-w-md">
                    ...
                    <Input
                        placeholder="Cari agenda, arahan, atau narahubung..."
                        value={globalFilter}
                            ...
                            <TableRow>
                                ...
                                >
                                    <div className="flex flex-col items-center gap-2 text-muted-foreground">
                                        ...
                                        <p>Belum ada data monitoring arahan direksi.</p>
                                    </div>
                                </TableCell>
                            </TableRow>
                        )}
