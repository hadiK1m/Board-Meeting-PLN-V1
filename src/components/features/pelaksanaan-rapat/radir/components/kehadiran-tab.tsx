// src/components/features/pelaksanaan-rapat/radir/components/kehadiran-tab.tsx
"use client";
...
// Type for attendance status: "hadir" | "kuasa" | "tidak_hadir"
export type AttendanceStatus = "hadir" | "kuasa" | "tidak_hadir";

export interface DirectorAttendance {
    status: AttendanceStatus;
    kuasaTo?: string; // Name of director receiving the kuasa
}
...
export function KehadiranTab({
    ...
    const getStatusBadge = (status: AttendanceStatus) => {
        switch (status) {
            ...
            case "kuasa":
                return <Badge className="bg-amber-500 text-[10px]">Kuasa</Badge>;
            case "tidak_hadir":
                return <Badge variant="secondary" className="text-[10px]">Tidak Hadir</Badge>;
        }
    };
            ...
            <Card>
                ...
                <CardContent>
                    <div className="grid gap-2">
                        {directorOptions.map((dir) => {
                            const attendance = attendanceData[dir.label] || { status: "hadir" as AttendanceStatus };
                            const isKuasa = attendance.status === "kuasa";

                            // Get other directors for kuasa dropdown (exclude self)
                            const otherDirectors = directorOptions.filter(d => d.label !== dir.label);

                            return (
                                <div
                                    key={dir.value}
                                    className={`p-3 rounded-lg border transition-all ${attendance.status === "hadir"
                                        ? 'bg-green-50 border-green-200'
                                        : attendance.status === "kuasa"
                                            ? 'bg-amber-50 border-amber-200'
                                            : 'bg-slate-50 border-slate-200'
                                        }`}
                                >
                                    <div className="flex items-center gap-3">
                                        <span className="text-sm font-medium flex-1">{dir.label}</span>
                                        <div className="flex items-center gap-2">
                                            ...
                                            <Select
                                                ...
                                                onValueChange={(value: AttendanceStatus) => {
                                                    if (value === "kuasa") {
                                                        onAttendanceChange(dir.label, { status: "kuasa", kuasaTo: "" });
                                                    } else {
                                                        onAttendanceChange(dir.label, { status: value });
                                                    }
                                                }}
                                            >
                                                ...
                                                <SelectContent>
                                                    ...
                                                    <SelectItem value="kuasa">Kuasa</SelectItem>
                                                </SelectContent>
                                            </Select>
                                        </div>
                                    </div>

                                    {/* Kuasa dropdown - shows when status is "kuasa" */}
                                    {isKuasa && (
                                        <div className="mt-3 pl-0">
                                            ...
                                            <Select
                                                value={attendance.kuasaTo || ""}
                                                onValueChange={(value) => {
                                                    onAttendanceChange(dir.label, {
                                                        status: "kuasa",
                                                        kuasaTo: value
                                                    });
                                                }}
                                            >
                                                <SelectTrigger className="w-full h-9 text-sm">
                                                    <SelectValue placeholder="Pilih penerima kuasa..." />
                                                </SelectTrigger>
                                                <SelectContent>
